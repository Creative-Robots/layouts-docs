...
const Linkable = ({id}:{id:string}) => {
    ...
    return (
        ...
        <a href={`#${id}`} onClick={(e) => scrollToElement(e, type)} className="-ml-8 flex items-center opacity-0 border-0 group-hover:opacity-100 bg-gray-50 rounded-md shadow size-fit p-1 hover:bg-background-2" aria-label="Navigate to header">
...
const Code = ({ children, id, ...props }: {children:ReactNode, id:string}) => {
  ...
  : <code className="size-fit text-wrap bg-hover text-gray-800 border border-gray-300">{children}</code>
)};

const Icon = (icon:string) => {
  switch (icon) {
    case 'rocket':
      return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-rocket text-textcolor-primary stroke-current"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/></svg>
    case 'display':
      return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-monitor text-textcolor-primary stroke-current"><rect width="20" height="14" x="2" y="3" rx="2"/><line x1="8" x2="16" y1="21" y2="21"/><line x1="12" x2="12" y1="17" y2="21"/></svg>
    case 'robot':
      return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-bot text-textcolor-primary stroke-current"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>
    case 'question':
      return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-circle-help text-textcolor-primary stroke-current"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>
    case 'code':
      return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-code-xml text-textcolor-primary stroke-current"><path d="m18 16 4-4-4-4"/><path d="m6 8-4 4 4 4"/><path d="m14.5 4-5 16"/></svg>
      default:
...
export default function MdxComponent({content}: MdxComponentProps) {
  ...
  const CardBox = ({  title, icon, id, children, ...props }: {children:ReactNode, id:string, title:string, icon:string}) => (
    <div className={Card} id={id} onClick={() => redirCard(title)}>
      {Icon(icon)}
      <span className="text-lg font-body mt-2 text-textcolor-primary">{title}</span>
      <span className="text-sm font-body text-textcolor-secondary -my-2">{children}</span>
    </div>
  );
