...
function MyCodeBlock({ code, language, withTitleBar=false }:MyCodeBlockProps) {
    const [state, copyToClipboard] = useCopyToClipboard();
    const copyCode = () => {
        copyToClipboard(code);
    };
  return (
    <div className='h-fit w-full max-w-full bg-gray-900 rounded-lg my-4 shadow-lg py-2 relative flex flex-row overflow-x-scroll'>
    <CodeBlock code={code} language={language}>
        <div className={cn("h-8 max-w-full w-full border-b border-gray-600 mb-2", withTitleBar ? "" : "hidden")}></div>
        <button
      ...
      <CodeBlock.Code className="flex-1 mx-2 overflow-x-scroll relative max-w-full">
        <div className="table-row relative max-w-full">
